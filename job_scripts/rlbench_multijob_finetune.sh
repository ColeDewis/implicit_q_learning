...
# Example usage:
# sbatch --time=07:00:00 --array=1-10:2 --export=path="$(pwd)" job_scripts/rlbench_multijob_finetune.sh 2 microwave microwave_data.h5 mujoco_config.py 1000000
...
CONFIG_NAME=${4:-mujoco_config.py}  # Default to "antmaze_finetune_config.py" if not provided
...
for ((i=0; i<STEP_SIZE; i++)); do
    ...
    tmux send-keys -t ${SESSION_NAME}:0.0 "python $path/train_finetune.py --env_name=$ENV_NAME --config=$path/configs/${CONFIG_NAME} --eval_episodes=100 --replay_buffer_size=2000000 --eval_interval=${EVAL_INTERVAL} --seed=$SEED --port=$PORT" C-m
