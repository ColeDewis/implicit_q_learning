...
for ((i=0; i<STEP_SIZE; i++)); do
    ...
    tmux send-keys -t ${SESSION_NAME}:0.0 "python -u $path/train_finetune.py --env_name=$ENV_NAME --config=$path/configs/${CONFIG_NAME} --learner=DDQN --eval_episodes=100 --eval_interval=${EVAL_INTERVAL} --seed=$SEED --port=$PORT --overrides=$OVERRIDE 2>&1 | tee -a $LEARNER_LOG_FILE" C-m
