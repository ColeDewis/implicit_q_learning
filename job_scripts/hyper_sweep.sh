...
# Example usage:
# sbatch --export=path="$(pwd)" hyper_sweep.sh 2 antmaze-large-play-v0 Ant_maze_hardest-maze_noisy_multistart_True_multigoal_False_sparse.hdf5 01:10:00
...
HYPERPARAM_FORMATTED=$(echo $HYPERPARAM | tr ',' '_')

# Training loop for multiple seeds per hyperparameter
for ((i=0; i<NUM_SEEDS; i++)); do
    SEED=$i  # Start seeds at 0
    python $path/train_offline.py --env_name=$ENV_NAME --config=$path/configs/antmaze_config.py --eval_episodes=100 --eval_interval=100000 --seed=$SEED --overrides=$HYPERPARAM
